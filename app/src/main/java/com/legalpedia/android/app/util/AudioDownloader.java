...
public class AudioDownloader {
    ...
    public AudioDownloader(String lastModified, int status) {
    ...
    public void downloadFile(String urlStr, String toFile, DownloadListener downloadListener) throws IOException {
        ...
        downloadListener.progressUpdate(new DownloadMessage("ResponseCode: " + connection.getResponseCode() + "; file length:" +
        ...
        if (!startNewDownload) {
            ...
            downloadListener.progressUpdate(new DownloadMessage("resume download to: " + toFile));
            // append to exist downloadedFile
            writer = new FileOutputStream(toFile, true);
        } else {
            downloadListener.progressUpdate(new DownloadMessage("new download to: " + toFile));
        ...
        try {
            ...
            while (getStatus() == DOWNLOADING && (count = in.read(buffer)) != -1) {
                ...
                // progress....
                downloadListener.progressUpdate(new DownloadMessage((int) (progressLength * 100 / fileLength)));
    ...
    private void prepareDownload(String urlStr, String toFile, DownloadListener downloadListener) throws IOException {
        downloadListener.progressUpdate(new DownloadMessage("prepare download ..........."));
        ...
        conn.disconnect();
        downloadListener
                .progressUpdate(new DownloadMessage("prepare finished .... start a new Download = " + startNewDownload));
    }
    ...
    private HttpURLConnection createConnection(String urlStr, DownloadListener downloadListener) throws IOException {
        downloadListener.progressUpdate(new DownloadMessage("create new connection ...."));
